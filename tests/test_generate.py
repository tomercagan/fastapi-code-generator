...
from fastapi_code_generator.parser import LocalRefFileMissingError
...
OPEN_API_LOCAL_REF_DIR_NAME = Path('openapi') / 'local_ref'
...
@freeze_time("2020-06-19")
def test_generate_local_ref():
    """Test using yaml file with a local file reference"""

    import os

    os.chdir(str(DATA_DIR / OPEN_API_LOCAL_REF_DIR_NAME))

    oas_file = Path('body_and_parameters.yaml')
...
def test_generate_local_ref_wrong_dir():
    """Test using yaml file with a local file reference, but running from "wrong" directory"""

    import os
    os.chdir(DATA_DIR)
    oas_file = DATA_DIR / OPEN_API_LOCAL_REF_DIR_NAME / 'body_and_parameters.yaml'

    with TemporaryDirectory() as tmp_dir:
        output_dir = Path(tmp_dir) / oas_file.stem

        with pytest.raises(LocalRefFileMissingError):
            generate_code(
                input_name=oas_file.name,
                input_text=oas_file.read_text(),
                output_dir=output_dir,
